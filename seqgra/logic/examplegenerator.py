...
class ExampleGenerator:    
    @staticmethod
    def generate_example(condition: Condition, background: Background, background_character: str = "_") -> Example:
        background: str = BackgroundGenerator.generate_background(background, condition)
        ...
        if random.uniform(0, 1) <= rule.probability:
            ...
            if rule.spacing_constraints is not None and len(rule.spacing_constraints) > 0:
                ...
                for spacing_constraint in rule.spacing_constraints:
                    example = ExampleGenerator.add_spatially_constrained_elements(example, spacing_constraint, \
                        elements[spacing_constraint.sequence_element1.id], \
                        elements[spacing_constraint.sequence_element2.id], rule.position)
            ...
            for element in elements.values(): 
                position: int = ExampleGenerator.get_position(rule.position, len(example.sequence), len(element))
                example = ExampleGenerator.add_element(example, element, position)

        return example
    
    @staticmethod
    def get_position(rule_position: str, sequence_length, element_length) -> int:
        if rule_position == "random":
            return np.random.randint(0, high=sequence_length - element_length + 1)
    ...
    @staticmethod
    def get_distance(example: Example, spacing_constraint: SpacingConstraint, element1: str, element2: str, rule_position: str) -> int:
        ...
        return np.random.randint(spacing_constraint.min_distance, high=min(spacing_constraint.max_distance, max_distance) + 1)

    @staticmethod
    def add_spatially_constrained_elements(example: Example, spacing_constraint: SpacingConstraint, element1: str, element2: str, rule_position: str) -> Example:
        ...
        position1: int = ExampleGenerator.get_position(rule_position, len(example.sequence), len(element1) + distance + len(element2))
    ...
    @staticmethod
    def add_element(example: Example, element: str, position: int, grammar_character: str = "G") -> Example:
        example.sequence = example.sequence[:position] + element + example.sequence[position + len(element):]
        example.annotation = example.annotation[:position] + (grammar_character * len(element)) + example.annotation[position + len(element):]
        return example