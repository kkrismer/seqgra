...
class BackgroundGenerator:
    ...
    @staticmethod
    def generate_background(background: Background, condition: Condition, set_name: str) -> str:
        bg_length: int = BackgroundGenerator.__determine_length(background)
        alphabet_distribution: AlphabetDistribution = BackgroundGenerator.__select_alphabet_distribution(background, condition, set_name)
    ...
    @staticmethod
    def __select_alphabet_distribution(background: Background, condition: Condition, set_name: str) -> AlphabetDistribution:
        if condition is None:
            # return set specific alphabet distribution, if exists
            for alphabet_distribution in background.alphabet_distributions:
                if alphabet_distribution.condition_independent and alphabet_distribution.set_name == set_name:
                    return alphabet_distribution
                    
            # or return global alphabet distribution, if exists
            for alphabet_distribution in background.alphabet_distributions:
                if alphabet_distribution.condition_independent and alphabet_distribution.set_independent:
                    return alphabet_distribution
